...
// Global variables
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
volatile char data0=0;	// Digit 0 data
volatile char data1=0;	// Digit 1 data
volatile char data2=0;	// Digit 2 data
volatile char data3=0;	// Digit 3 data
volatile char receiveCount = 0;	// Will count from 0 to 3
volatile char brightDelay = 0;	// Read from EEPROM on startup
volatile char DPStatus = 0x00;	// Decimal point status, each bit represents one DP
volatile char uartMode = 0;
volatile char spiMode = 1;
volatile char prevData0=0;	// Previous Digit 0 data
volatile char prevData1=0;	// Previous Digit 1 data
volatile char prevData2=0;	// Previous Digit 2 data
volatile char prevData3=0;	// Previous Digit 3 data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
int main(void)
{
	...
	while(1)
	{
		...
		delay_us(DIGIT_ON_TIME);
		clearDisplay();
		delay_us((uint16_t) brightDelay*BRIGHT_FACTOR);
	}
}
